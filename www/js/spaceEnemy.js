...
const ENEMY_COUNT = 6;     // desktop budget
...
export function createSpaceEnemies({ scene, archery, center = { x: 0, z: 0 }, onEvent = () => {}, playHitSound = null }) {
  ...
  const enemyBudget = mobile ? 4 : ENEMY_COUNT;
  const mats = {};
  ...
  for (let i = 0; i < enemyBudget; i++) spawn(i);
  ...
  function update(dt, elapsed) {
    ...
    for (const e of enemies) {
      ...
      // Enemies near the dome centre remain responsive; outer, high-up
      // enemies are visually stable enough at 30Hz on touch devices.
      const dx = e.mesh.position.x - center.x;
      const dz = e.mesh.position.z - center.z;
      const far = dx * dx + dz * dz > 34 * 34;
      if (!mobile || !far || !(updateFrame & 1)) {
        e.phase += dt * 0.6;
